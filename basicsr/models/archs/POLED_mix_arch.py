...
class POLED(nn.Module):
    def __init__(self, in_nc, out_nc, nf=32, ns=64, kpn_sz=15, norm_type=None, \
        ...
        if self.wiener_level >= 4:
            self.wiener_kernel_4 = init_wiener_kernel(self.croped_ksz, ns * 4)
        ...
        # Encoder
        self.conv_1 = nn.Conv2d(12, ns, 3, 1, 1, bias=True)  # 64f
        basic_block_64 = functools.partial(ResidualBlock_noBN, nf=ns)
        ...
        basic_block_128 = functools.partial(ResidualBlock_noBN, nf=ns * 2)
        ...
        basic_block_256 = functools.partial(ResidualBlock_noBN, nf=ns * 4)
        self.encoder3 = make_layer(basic_block_256, 2)  # 256f
        if ns != nf:
            self.wiener_fix_KPN_3 = nn.Conv2d(self.ns * 4, nf * 4, 1, 1, 0, bias=True)

        self.conv_4 = nn.Conv2d(ns * 4, ns * 4, 3, 1, 1, bias=True)  # 256f
        if ns != nf:
            self.wiener_fix_KPN_4 = nn.Conv2d(self.ns * 4, 256, 1, 1, 0, bias=True)

        self.conv_5 = nn.Conv2d(ns * 4, ns * 4, 3, 1, 1, bias=True)       # 256f

        # Decoder
        self.conv_6 = nn.Conv2d(ns * 4, ns * 4, 3, 1, padding=1, bias=True)
        self.conv_7 = nn.Conv2d(ns * 8, ns * 8, 3, 1, padding=1, bias=True)
        self.conv_8 = nn.Conv2d(ns * 8, ns * 8, 3, 1, padding=1, bias=True)
        self.conv_9 = nn.Conv2d(ns * 6, ns * 4, 3, 1, padding=1, bias=True) ##
        self.conv_10 = nn.Conv2d(ns * 4, ns * 4, 3, 1, padding=1, bias=True)
        self.conv_11 = nn.Conv2d(ns * 3, ns * 2, 3, 1, padding=1, bias=True) ##
        self.conv_12 = nn.Conv2d(ns * 2, ns * 2, 3, 1, padding=1, bias=True)
        self.conv_13 = nn.Conv2d(int(ns * 1.5), ns, 3, 1, padding=1, bias=True)
        self.conv_14 = nn.Conv2d(ns, 12, 3, 1, padding=1, bias=True)
        ...
        self.St_block = DWTForward()
        self.Ed_block = DWTInverse()

        if final_kernel:
            self.final_mend = nn.Conv2d(12, 12, 3, 1, padding=1, bias=True)
        ...
        if self.kernel_cond:
            ...
            self.Coefficient_body_4 = nn.Sequential(
                conv_block(ns * 4, 8 * nf, kernel_size=kernel_size),
            ...
            # Basis Decoder:
            # skip's shape should be same as channel_in
            self.Basis_body_1 = Kernel_UpBlock(channel_in=ns * 4, block_channels=4 * nf, act_type=act_type,
                                               upscale_factor=2)
            ...
            self.Basis_tail_4 = Basis_Decoder_Tail(channel_in=1 * nf, channel_out=8 * nf, basis_num=self.basis_num, kernel_size=self.kpn_sz, act_type='prelu')

            if self.final_kernel:
                self.Basis_tail_4_final = Basis_Decoder_Tail(channel_in=1 * nf, channel_out=12,
    ...
    def forward(self, x, flags=None):
        ...
        if self.wiener_level >= 1:
            if self.training:
                wiener_kernels_1 = self.wiener_kernel_1.repeat(x.shape[0], 1, 1, 1)
        ...
        if self.wiener_level >= 2:
            if self.training:
                wiener_kernels_2 = self.wiener_kernel_2.repeat(x.shape[0], 1, 1, 1)  # N ns*2 h w
        ...
        if self.wiener_level >= 4:
            if self.training:
                wiener_kernels_4 = self.wiener_kernel_4.repeat(x.shape[0], 1, 1, 1)  # N ns*4 h w
        ...
        # 1. get coeffs:
        mid_kfea4 = self.Coefficient_body_4(fea) #+ kfea4
        ...
        kernels_4 = kernels_4.reshape([-1, self.kpn_sz * self.kpn_sz * (fea.shape[1]) * 2, fea.shape[2], fea.shape[3]])
        ...
        if self.final_kernel:
            ...
            kernels_final = kernels_final.reshape([-1, self.kpn_sz * self.kpn_sz * (fea_x.shape[1]) * 2, fea_x.shape[2], fea_x.shape[3]])
...
class ResidualBlock_noBN_wiener(nn.Module):
    '''Residual block w/o BN
    ---Conv-ReLU-Conv-+-
     |________________|
    '''

    def __init__(self, nf=64*3):
        super(ResidualBlock_noBN_wiener, self).__init__()
        self.conv1 = nn.Conv2d(nf, nf, 3, 1, 1, bias=True, groups=3)
        self.conv2 = nn.Conv2d(nf, nf, 3, 1, 1, bias=True, groups=3)
...
def get_clear_feature_test(feature_in, kernels):
    device = feature_in.device
    clear_features = torch.zeros(feature_in.size(), device=device) # .to(device)
    ks = kernels.shape[2]
    first_scale_inblock_pad = F.pad(feature_in, (ks, ks, ks, ks), "replicate")
...
class DWTForward(nn.Module): # DWT: feature map / 2, channel * 4
    ...
    def __init__(self):
        ...
        filts = np.stack([ll[None, ::-1, ::-1], lh[None, ::-1, ::-1],
                         ...
                         axis=0)
        self.weight = nn.Parameter(torch.tensor(filts).to(torch.get_default_dtype()), requires_grad=False)
...
class DWTInverse(nn.Module): # IDWT: feature map * 2, channel / 4
    def __init__(self):
