...
class POLEDTrainModel(BaseModel):
    ...
    def init_training_settings(self):
        ...
        # ssim loss
        if train_opt.get('extra_opt'):
            extra_type = train_opt['extra_opt'].pop('type')
            cri_extra_cls = getattr(loss_module, extra_type)
            self.cri_extra = cri_extra_cls(**train_opt['extra_opt']).to(self.device)
        else:
            self.cri_extra = None
        
        # LPIPS
        if train_opt.get('extra_2_opt'):
            extra_2_type = train_opt['extra_2_opt'].pop('type')
            cri_extra_2_cls = getattr(loss_module, extra_2_type)
            self.cri_extra_2 = cri_extra_2_cls(**train_opt['extra_2_opt']).to(self.device)
        else:
            self.cri_extra_2 = None
    ...
    def optimize_parameters(self, current_iter):
        ...
        if self.flags is None:
            N,_,_,_ = self.lq.shape
            self.flags = torch.zeros([N,3]).to(self.lq.device)
        ...
        # extra loss
        if self.cri_extra:
            l_extra = self.cri_extra(self.output, self.gt)
            l_total += l_extra
            loss_dict['l_extra'] = l_extra
            
        # extra loss 2
        if self.cri_extra_2:
            l_extra_2 = self.cri_extra_2(self.output, self.gt)
            l_total += l_extra_2
            loss_dict['l_extra_2'] = l_extra_2
