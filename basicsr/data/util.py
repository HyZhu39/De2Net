...
def paired_paths_PSF_from_meta_info_file(folders, keys, meta_info_file,
    ...
    with open(meta_info_file, 'r') as fin:
        pair_names = [line.replace('\n', '').split(' ') for line in fin]

    paths = []
    #print("pair_names", pair_names[0])
...
def paired_paths_classidx_Colorization_ImageNet(folders, keys, meta_info_file,
                                     filename_tmpl):
    assert len(folders) == 1, (
        'The len of folders should be 1 with [rgb_folder]. '
        f'But got {len(folders)}')
    assert len(keys) == 1, (
        'The len of keys should be 1 with [rgb_key]. '
        f'But got {len(keys)}')
    path_to_folder = folders[0]
    #print('path_to_folder', path_to_folder)
    with open(meta_info_file, 'r') as fin:
        names_folders_idx = [line.replace('\n', '').split(' ') for line in fin]
    paths = []
    for rgb_name, folder_name, classidx in names_folders_idx:
        #print('rgb_name', rgb_name) # n04238763_7341
        #print('folder_name', folder_name) # n04238763
        #print('classidx', classidx) # 798
        
        basename = rgb_name
        ext = '.JPEG'

        input_name = f'{filename_tmpl.format(basename)}{ext}'
        rgb_path = osp.join(path_to_folder, folder_name, input_name)
        #print('rgb_path', rgb_path)
        paths.append(
            dict([(f'rgb_path', rgb_path),
